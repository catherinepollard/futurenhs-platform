...
kustomize build "$CURRENT_DIR/../kubernetes/ingress/step1/overlays/$ENV" --reorder none |
  sed \
    -e "s/--USERNAME--/$NAME/g" \
    -e "s/--DNS_PREFIX--/$NAME/g" \
    -e "s/--REGION--/$REGION/g" \
    -e "s/--EMAIL--/$EMAIL/g" |
  kubectl apply -f -
...
kustomize build "$CURRENT_DIR/../kubernetes/ingress/step2/overlays/$ENV" --reorder none |
  sed \
    -e "s/--USERNAME--/$NAME/g" \
    -e "s/--DNS_PREFIX--/$NAME/g" \
    -e "s/--REGION--/$REGION/g" \
...
printf "\nhttps://$NAME.$REGION.cloudapp.azure.com/hello/$NAME\n\n"
